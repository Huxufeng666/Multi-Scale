...
# class CristCoistAttention(nn.Module): # (占位符)
...
class ChannelAttention(nn.Module):
    def __init__(self, in_channels):
        ...
        self.fc = nn.Sequential(nn.Linear(in_channels, in_channels // 16, bias=False), nn.ReLU(inplace=False),
...
class SpatialAttention(nn.Module): # (您上一节修改后的版本)
    def __init__(self, in_channels):
        super().__init__()
        self.fuse_conv = nn.Sequential(nn.Conv2d(in_channels * 2, in_channels, 1, bias=False), nn.BatchNorm2d(in_channels), nn.ReLU(inplace=False))
...
class DFSANet(nn.Module):
    def __init__(self, n_channels=1, n_classes=1, base_c=64):
        ...
        self.up_Simple1 = SimpleUp(base_c  , 64  , use_skip=False)
        ...
        # (我们假设是 c1, ef2_out, e1)
        self.ef1 = EfficientFeature([base_c, base_c, base_c], 64)           # out: 64
        ...
        self.dec1 = Up(base_c , base_c , base_c)      # (128+128) -> 64
    ...
    def forward(self, x, gt=None):
        ...
        # 上采样到原始尺寸
        final_output = F.interpolate(final_logits, size=x.shape[2:], mode='bilinear', align_corners=False)
